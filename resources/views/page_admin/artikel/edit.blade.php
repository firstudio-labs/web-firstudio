...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-2"><span class="text-muted fw-light">Artikel /</span> Edit Artikel</h4>
        @auth
        <div class="mb-3">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('judul') is-invalid @enderror"
                                            id="judul" name="judul" value="{{ $artikel->judul }}" required>
                                        @error('judul')
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('penulis') is-invalid @enderror"
                                            id="penulis" name="penulis" value="{{ $artikel->penulis }}" required>
                                        @error('penulis')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        @if($artikel->gambar)
                                            <div class="mb-2">
                                                <img src="{{ asset('storage/artikel/' . $artikel->gambar) }}" alt="Preview"
                                                    class="img-thumbnail" style="max-width: 200px;">
                                            </div>
                                        @endif
                                        <input type="file" class="form-control @error('gambar') is-invalid @enderror"
                                            id="gambar" name="gambar">
                                        <small class="text-muted">Format: jpeg, png, jpg, gif, svg. Maksimal 2MB. Biarkan kosong jika tidak ingin mengubah gambar</small>
                                        @error('gambar')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="d-flex gap-2 mb-2">
                                            ...
                                            <button type="button" class="btn btn-outline-warning btn-sm ai-feature-btn" id="btnEnhanceContent" title="Tingkatkan konten yang sudah ada dengan AI" {{ ($aiRemainingTokens ?? 0) <= 0 ? 'disabled' : '' }}>
                                                <i class="bx bx-magic-wand"></i> Enhance Konten
                                            </button>
                                        </div>
                                        <textarea class="form-control @error('isi') is-invalid @enderror" id="isi"
                                            name="isi" required>{{ $artikel->isi }}</textarea>
                                        <small class="text-muted">Tip: Klik "Generate dengan AI" untuk membuat konten baru, atau "Enhance Konten" untuk memperbaiki konten yang sudah ada</small>
                                        @error('isi')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('catatan') is-invalid @enderror"
                                            id="catatan" name="catatan" value="{{ $artikel->catatan }}">
                                        @error('catatan')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('status') is-invalid @enderror" id="status"
                                            ...
                                            <option value="Publik" {{ $artikel->status == 'Publik' ? 'selected' : '' }}>Publik
                                            </option>
                                        ...
                                        <div class="card-body">
                                            <div id="preview-container" class="article-preview">
                                                <div class="article-header mb-4">
                                                    <h1 id="preview-judul" class="article-title mb-3">{{ $artikel->judul }}
                                                    </h1>
                                                </div>
                                                <div id="preview-gambar" class="article-image mb-4">
                                                    @if($artikel->gambar)
                                                ...
                                                <div class="article-meta text-muted mb-3">
                                                    <span class="me-3">
                                                        ...
                                                        <span id="preview-penulis">{{ $artikel->penulis }}</span>
                                                    </span>
                                                    <span class="me-3">
                                                        <i class="bx bx-category"></i>
                                                        <span
                                                            id="preview-kategori">{{ $artikel->kategoriArtikel->kategori_artikel }}</span>
                                                    </span>
                                                </div>
                                                <div id="preview-isi" class="article-content">{!! $artikel->isi !!}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="alert alert-warning">
                        ...
                        <strong>Perhatian:</strong> Generate konten baru akan mengganti semua isi artikel yang sudah ada. Gunakan fitur "Enhance Konten" jika ingin memperbaiki konten yang sudah ada.
                    </div>
                    <div class="mb-3">
    ...
    <!-- Modal Enhance Content -->
    <div class="modal fade" id="modalEnhanceContent" tabindex="-1" aria-labelledby="modalEnhanceContentLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalEnhanceContentLabel">
                        <i class="bx bx-magic-wand"></i> Enhance Konten Artikel
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="alert alert-info">
                        <i class="bx bx-info-circle"></i>
                        AI akan memperbaiki dan memperluas konten yang sudah ada tanpa menghilangkan inti artikel.
                    </div>
                    <div class="mb-3">
                        <label for="enhanceType" class="form-label">Jenis Enhancement</label>
                        <select class="form-select" id="enhanceType">
                            <option value="improve">Perbaiki Grammar & Style</option>
                            <option value="expand">Perluas Konten & Detail</option>
                            <option value="seo">Optimasi SEO</option>
                            <option value="restructure">Restructure & Format</option>
                        </select>
                    </div>
                    <small class="text-muted">
                        Enhancement akan mempertahankan struktur dan inti artikel yang sudah ada.
                    </small>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                    <button type="button" class="btn btn-warning" id="btnSubmitEnhanceContent">
                        <i class="bx bx-loader-alt bx-spin" id="loadingEnhance" style="display:none;"></i>
                        Enhance Konten
                    </button>
                </div>
            </div>
...
@section('scripts')
    ...
    <script>
        ...
        document.getElementById('btnGenerateAI').addEventListener('click', function() {
            ...
            // Peringatan untuk replace content
            if (editor && editor.getData().trim()) {
                if (!confirm('Generate konten baru akan mengganti semua isi artikel yang sudah ada. Lanjutkan?')) {
                    return;
                }
            }
        ...
        // Enhance Content Modal
        document.getElementById('btnEnhanceContent').addEventListener('click', function() {
            if (!editor || !editor.getData().trim()) {
                alert('Tidak ada konten untuk di-enhance. Silakan isi konten terlebih dahulu.');
                return;
            }
            
            const modal = new bootstrap.Modal(document.getElementById('modalEnhanceContent'));
            modal.show();
        });

        // Submit Generate Titles (sama dengan create page)
        ...
        document.getElementById('btnSubmitGenerateAI').addEventListener('click', function() {
                ...
                if (data.success) {
                    ...
                    if (typeof data.remaining_tokens !== 'undefined') {
                        ...
                        if (data.remaining_tokens <= 0) {
                            ...
                            document.getElementById('btnEnhanceContent')?.setAttribute('disabled', 'disabled');
                        }
                    }
                } else {
            ...
            .finally(() => {
                btn.disabled = false;
                loading.style.display = 'none';
                btn.innerHTML = '<i class="bx bx-brain"></i> Generate Konten Baru';
            });
        });

        // Submit Enhance Content
        document.getElementById('btnSubmitEnhanceContent').addEventListener('click', function() {
            const currentContent = editor.getData();
            const enhanceType = document.getElementById('enhanceType').value;
            const judul = document.getElementById('judul').value;

            if (!currentContent.trim()) {
                alert('Tidak ada konten untuk di-enhance');
                return;
            }

            const btn = this;
            const loading = document.getElementById('loadingEnhance');
            
            // Show loading state
            btn.disabled = true;
            loading.style.display = 'inline-block';
            btn.innerHTML = '<i class="bx bx-loader-alt bx-spin"></i> Enhancing...';

            fetch('{{ route("admin.artikel.enhance-content") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            ...
            .then(data => {
                if (data.success) {
                    // Set enhanced content to CKEditor
                    editor.setData(data.content);
                    
                    // Close modal
                    const modal = bootstrap.Modal.getInstance(document.getElementById('modalEnhanceContent'));
                    modal.hide();
                    
                    // Show success message
                    showSuccessNotification(`Konten berhasil di-enhance! (${data.word_count} kata)`);
                    if (typeof data.remaining_tokens !== 'undefined') {
                        const el = document.getElementById('aiRemainingTokens');
                        if (el) el.textContent = data.remaining_tokens;
                        if (data.remaining_tokens <= 0) {
                    ...
                    showErrorNotification((data.message || 'Gagal enhance konten') + ' Halaman akan di-refresh dalam 3 detik...');
            ...
            .catch(error => {
                console.error('Error:', error);
                showErrorNotification('Terjadi kesalahan saat enhance konten. Halaman akan di-refresh otomatis dalam 3 detik...');
            ...
            .finally(() => {
                btn.disabled = false;
                loading.style.display = 'none';
                btn.innerHTML = '<i class="bx bx-magic-wand"></i> Enhance Konten';
            });
        });

        // Display generated titles function
